##!/usr/bin/env python3
# -*- coding: utf-8 -*-

from math import exp, cos
from scipy.stats import truncexpon
from time import time
import random
import numpy as np 
#Escreva seu nome e numero USP
INFO = {12556819:"Heloisa Tambara"}
A = 0.3 # A = 0.rg
B = 0.4  # B = 0.cpf




def f(x):
    """
    Esta funcao deve receber x e devolver f(x), como especifcado no enunciado
    Escreva o seu codigo nas proximas linhas
    """
    A = 0.3# A = 0.rg
    B = 0.4  # B = 0.cpf
    f = exp(-A*x) * cos(B*x)
    
    return f




# Crude
def crude(Seed = None):
    random.seed(Seed)
    """
    Esta funcao deve retornar a sua estimativa para o valor da integral de f(x) entre 0 e 1
    usando o metodo crude
    Escreva o seu codigo nas proximas linhas
    """

    global n #inserir n

    # calcula o crude
    soma = 0
    x = np.random.uniform(0,1,n)
    for i in range(n): # gerar n numeros x, calcular suas f(x), somar tudo e dividir por n
        soma += f(x[i])
    mu = soma/n

    return mu # Retorne sua estimativa

# converge

#


# Hit or Miss
def hit_or_miss(Seed = None):
    random.seed(Seed)
    """
    Esta funcao deve retornar a sua estimativa para o valor da integral de f(x)
    usando o metodo hit or miss
    Escreva o seu codigo nas proximas linhas
    """

    global n # inserir n
    soma = 0
    x, y = np.random.uniform(0,1,n), np.random.uniform(0,1,n) # gera valor posicional
    for i in range(n): 
        if y[i] <= f(x[i]):
            soma += 1 # sucesso se estiver abaixo da linha da função
    
    mu = soma/n # calcula media 

    return mu # Retorne sua estimativa



# Control Variate

# funcao que aproxima f(x) no intervalo e sua integral
def phi(x): return -0.3898874043587 * x + 1
def Phi(x): return -0.3898874043587/2 *x**2+x
    


def control_variate(Seed = None):
    random.seed(Seed)
    """
    Esta funcao deve retornar a sua estimativa para o valor da integral de f(x)
    usando o metodo control variate
    Escreva o seu codigo nas proximas linhas
    """


    global n # inserir n
    soma = 0
    x =  np.random.uniform(0,1,n) # gera valor para x
    for i in range(n):
        soma += f(x[i]) - phi(x[i]) # diferenca em cada ponto sorteado
    mu = soma/n + Phi(1) - Phi(0) # media das diferencas e integral de phi no intervalo
    return mu # Retorne sua estimativa



# Importance Sampling

# funcao que descreve a curva da disribuicao
def probexp(x, a=1, b=1):
    randg = a*exp(-a*x)/(1-exp(-b))
    return randg



def importance_sampling(Seed = None):
    random.seed(Seed)
    """
    Esta funcao deve retornar a sua estimativa para o valor da integral de f(x)
    usando o metodo importance sampling
    Escreva o seu codigo nas proximas linhas
    """

    global n # a definir

    soma = 0
    x = [truncexpon.rvs(b = 0.5, scale = 2) for i in range(n)] # gera valores randomicos segundo a distribuicao escolhida aproximada
    for i in range(n):
        soma += f(x[i])/probexp(x[i],0.5, 0.5) # media ponderada

    mu = soma/n

    return mu #Retorne sua estimativa





# convergências
def convergeCrude():
    global n
    x = [crude() for i in range(100)]
   # print(x[1:10])
    x2 = [X**2 for X in x]
    mean = (sum(x)/100)
    var = sum(x2)/100 - mean**2
    return var, mean

def convergeHoM():
    global n
    x = [hit_or_miss() for i in range(100)]
   # print(x[1:10])
    x2 = [X**2 for X in x]
    mean = (sum(x)/100)
    var = sum(x2)/100 - mean**2
    return var, mean

def convergeCV():
    global n
    x = [control_variate() for i in range(100)]
   # print(x[1:10])
    x2 = [X**2 for X in x]
    mean = (sum(x)/100)
    var = sum(x2)/100 - mean**2
    return var, mean

def convergeIS():
    global n
    x = [importance_sampling() for i in range(100)]
   # print(x[1:10])
    x2 = [X**2 for X in x]
    mean = (sum(x)/100)
    var = sum(x2)/100 - mean**2
    return var, mean




def main():
    #Coloque seus testes aqui
    print(f'crude: {crude()}\n')
    print(f'hit or miss: {hit_or_miss()}\n')
    print(f'control variate: {control_variate()}\n')
 #   print(f'importance sampling: {importance_sampling()}\n')


if __name__ == "__main__":
    print('Pseudo Random')
    # ver quando converge para crude
    t0 = time()
    n = 1
    a = convergeCrude()
    while 1:
        a = convergeCrude()
        if n >= 15366400*a[0]/(a[1]**2):
            break
        n *= 2
    t1 = time() - t0
    print(n, a[1], 'Calculado em', t1, 'segundos')



    # ver quando converge para hit or miss
    t0 = time()
    n = 1
    a = convergeHoM()
    while 1:
        a = convergeHoM()
        if n >= 15366400*a[0]/(a[1]**2):
            break
        n *= 2
    t1 = time() - t0
    print(n, a[1], 'Calculado em', t1, 'segundos')



    # ver quando converge para control variate
    t0 = time()
    n = 1
    a = convergeCV()
    while 1:
        a = convergeCV()
        if n >= 15366400*a[0]/(a[1]**2):
            break
        n*=2
    t1 = time() - t0
    print(n, a[1], 'Calculado em', t1, 'segundos')


    # ver quando converge para importance sampling
    t0 = time()
    n = 1
    a = convergeIS()
    while 1:
        a = convergeIS()
        if n >= 15366400*a[0]/(a[1]**2):
            break
        n*=2
    t1 = time() - t0
    print(n, a[1], 'Calculado em', t1, 'segundos')

   # main()     
